...
## 第十六批：Dispatch Preview 与 Approval Lock

`.github/workflows/benchmark-producer-preview.yml` 将 official 实验的成本/恢复决策从真实执行 workflow 中拆出。Preview 只读取仓库与 Actions artifact metadata，不引用 Provider/Tavily Secrets，也不运行 live graph。输入与 official Producer 对齐：model、lineage mode、history run、resume run 以及 `max_new_shards / max_new_search_calls / max_new_research_tokens`。

Preview 先生成与真实 Producer 同口径的 immutable `ProducerPlan`；continue 模式随后读取 history run artifact inventory 并要求存在有效 `deepscout-experiment-bundles`，resume 模式则恢复旧 `deepscout-producer-plan` 并读取 shard inventory。`plan_producer_resume()` 仍负责 fingerprint、finalized source、expired/pagination 与三层预算校验。

新增 `ProducerDispatchApproval`。approval digest 的 payload 绑定 current producer fingerprint、resume run ID、三层预算、reused/new 计数、new-run/search/token 上界和完整 20-shard reuse/execute 决策。run ID/attempt/experiment ID 属于执行实例身份，不参与 producer compatibility fingerprint；因此 Preview 与 actual Producer 可以跨 Actions run 复现相同 digest，但实验定义、lineage、可复用 shard 或预算的任何变化都会使 approval 失效。

真实 Producer 的 official 输入新增 `approval_digest`，且要求严格 64 位小写十六进制。preflight 在 live Provider/Tavily 调用前重新读取 history/resume inventory、重建 Resume Plan，再执行 approval 校验；mismatch 使用 exit 2 阻断。这样 Preview 之后 artifact 过期、shard 数变化、代码 SHA 更新、模型切换或预算调整都不能沿用旧批准。

Secrets 作用域同时收紧：preflight job-level env 不再持有 API key，只有 `Run live Provider/Tavily preflight` step 注入 Tavily 与 Provider secrets。Preview workflow 完全没有 `secrets.*`。这把“审计/批准计划”和“实际花费 API 配额”分成清晰的权限边界。

本地 CLI E2E 证明 preview/actual 不同 run identity 下 approval digest 一致；错误 digest 明确 exit 2。focused producer/repeated tests 为 21/21，全量真实 Redis 测试为 143/143。远端 Preview 本身仍未在无 Provider 凭据条件下代替真实实验结果；其职责只是生成成本与恢复计划。

## 当前验证边界

第一批使用 synthetic final-state 与 fake streamed graph 验证指标和轨迹；第二批使用 synthetic profile graph 验证 Settings/Graph 消融语义；第三批使用可控重复 synthetic graph 验证重复统计；第四批用可手算 paired fixture 验证 exact sign-flip、Wilcoxon、Cohen’s dz、rank-biserial、Cliff’s delta、Holm/BH 与分辨率；第五批验证 20-case Corpus 多样性、exact-Holm 可达性、paired-normal power/MDE 单调性和规划报告；第六批验证 source/rubric 完整性、域名/freshness 策略与 Corpus balance guard；第七批验证 Evidence metadata、source-policy compliance、双评审 agreement 与 adjudication；第八批验证 sidecar 绑定、缺失值语义、profile/case 聚合、human-gold paired delta/significance 与报告产物；第九批验证 bundle hash/identity、tamper detection、compatibility gate、thresholded regression 与 CLI exit semantics；第十批验证多 bundle discovery、duplicate ID rejection、invalid exclusion、last-known-good lineage、history gate 与 dashboard 产物；第十一批验证 promotion policy、manual override、latest decision、stale decision rejection、retention lineage closure 与安全 apply；第十二批验证 strict/relaxed Registry integrity、accepted/regression release gate、policy-as-code、统一审计 artifact 与非破坏性 retention preview；第十三批验证 reusable workflow contract、当前/历史 run artifact 下载路径、production-only runtime、artifact round-trip 与 CI smoke orchestration；第十四批验证 producer protocol guards、20-shard/600-unit merge、profile rotation 重建、duplicate/missing/path-escape guards 与 lineage 继承 contract；第十五批验证不可变 Producer Plan、跨失败 run shard reuse、shard/search/token 三层预算、单 shard canonical validation、provenance closure、finalized source rejection 与 17-reused/3-executed 恢复管线；第十六批验证 Dispatch Approval digest、Preview/actual 跨 run 稳定性、history lineage artifact 前置校验、approval mismatch exit 2 与 Secrets 最小作用域。这些 fixture 只证明实验基础设施正确，不代表真实 Provider 的 Benchmark/Ablation/统计成绩。
